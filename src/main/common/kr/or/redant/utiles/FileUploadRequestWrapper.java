...
public class FileUploadRequestWrapper extends HttpServletRequestWrapper {
   ...
   private void parsing(HttpServletRequest request, int thresold, int sizeMax) {
      ...
      if(this.multipartFlag){
         ...
         //클라이언트: input name = mem_id , mem_pass, mem_name ......
            ...
            if(item.getSize()>0){
               ...
               if(item.isFormField()){
                  ...
                  if(values ==null){
                     ...
                     //스왈로우 카피 : 원본 배열의 각각의 인덱스에서 레퍼렌스하는 요소값의 주소가 복사 대상 배열에 제공(원본 배열의 요소 값 주소가 복사대상 배열의 요소값 주소와 동일) 
                     //          원본배열[0] ==> (0x34f5)memberInfo(MemberVO)
                     //          복사대상배열[0] ==> (0x34f5)
                     //딥 카피 : 원본 배열의 각각의 인덱스에서 레퍼런스하는 요소값의 주소와 
                     //      복사 대상 배열의 인덱스에서 레퍼런스하는 요소값의 주소가 상이 
